...
def get_ngbxs_nsupers_runs() -> dict:
    ngbxs_nsupers_runs = {
        ...
        (32768, 128): 2,
        (65536, 128): 2,
        (131072, 128): 2,  # no serial
        (262144, 128): 2,  # cuda only
        (524288, 128): 2,  # cuda only
        (1048576, 128): 2,  # cuda only
    }
...
def get_ngbxs_nsupers_nthreads(
    ...
    if buildtype == "serial":
        ...
        ngbxs_nsupers_nthreads = {
            ...
            (65536, 128): [256, 128, 64, 16, 8],  # no threads=8
            (131072, 128): [256, 128, 64],  # no serial & no threads=64
            (262144, 128): [256, 64],  # cuda only
            (524288, 128): [256, 64],  # cuda only
            (1048576, 128): [256],  # cuda only
        }
